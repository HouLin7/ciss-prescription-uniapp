<template>
	...
	<view class="counter">
		...
		<view v-if="!hasLogin" style="display: flex;flex-direction: column;padding-top: 20rpx;align-items: center;">
			...
			<view class="uni-label-text">登陆后才可以发布</view>
...
<script lang="ts">
	import { cityList, areaList, provinceList } from "../mock_data/mock-test-data.js";
	...
	import { getMyLocation } from '../api/location_api';
			...
			getPoiInfo(latitude : number, longitude : number) {
				...
				amapPlugin.getRegeo({
					...
					success: (data : AmapPoiItem[]) => {
						...
						let keyword = addressInfo.regeocodeData.addressComponent.province;
						if (keyword.includes("北京") || keyword.includes("天津") || keyword.includes("上海")
							|| keyword.includes("重庆")) {
							this.city = keyword.substring(0,2);
						} else {
							this.city = keyword;
						}
